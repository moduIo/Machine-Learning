...
% Main()
%--------------------------------------------
function driver(loss, training_path, test_path, k, C, a, T, B, validation_size, keep, sample, regularizer, lambda, Xtrain, Xtest, ytrain)
    ...
    if strcmp('Least Squares', loss)
        ...
        [w, validation_error] = train_sgd(loss, Xtrain, ytrain, C, a, T, B, validation_size, keep);
            ...
            for j = i: i + B
                if j <= m_train                
                    ...
                    elseif strcmp('SVM Squared Hinge', loss)
                        if y(r) * (X(r, :) * w_t) <= 1
                            w_t = w_t + (eta * C * ((2 * y(r) * X(r, :)') - (2 * (X(r, :)' * (X(r, :) * w_t)))));
                        end
...
function X = generate_cross_features(X, k)
    ...
    for i = 1: m
        ...
        % Raise cross terms to power 2..k
        for t = 1: k - 2
            for j = 1: individual_d
